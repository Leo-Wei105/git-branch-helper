...
import { BranchPrefix, ConfigurationSchema, DateFormat } from './types';
...
export class ConfigManager {
     ...
     * 获取配置
     */
    getConfiguration(): ConfigurationSchema {
    ...
    async addBranchPrefix(prefix: string, description: string, isDefault: boolean = false): Promise<void> {
        const validation = Utils.validatePrefix(prefix);
    ...
    private async addPrefixInteractive(): Promise<void> {
        const prefix = await vscode.window.showInputBox({
            ...
            validateInput: (value) => {
                const validation = Utils.validatePrefix(value);
